...
# sha256 always produces a unique output, but we mod its output, so we still
# get collisions
...
def how_many_before_collision(num_buckets):
    ...
    while True:
        ...
        key = random.random()
        # hash them, mmodulo them by number of buckets aka size of the array
